...
import { getClientTimeline } from '@/lib/crm/actions';
...
export default function ClientTimeline({ clientId }: ClientTimelineProps) {
    ...
    useEffect(() => {
        let isMounted = true;
        setLoading(true);
        getClientTimeline(clientId).then(data => {
            if (isMounted) {
                setEvents(data);
                setLoading(false);
            }
    ...
    return (
        <div className="relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-neutral-200 dark:before:via-neutral-700 before:to-transparent pt-2 pb-6">
